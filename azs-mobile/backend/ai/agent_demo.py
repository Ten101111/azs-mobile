...
from .agent.claims import TITLES as TYPE_TITLES
...
def build_scenarios() -> dict[str, dict]:
    ...
    def top_final(evidence: str) -> dict:
        return {
            ...
            # Рекомендаций не просили — actions пусто (решение владельца 23.09.2026).
            "why": [], "where": [], "actions": [],
    ...
    def why_final(evidence: str) -> dict:
        ...
        why = [{"text": f"Возможно, часть изменения объясняется числом работающих АЗС: {s['stations_prev']} → {s['stations_cur']}; "
                        f"вклад этого фактора — {_fmt(s['stations_share'], 1)} % изменения, остальное ({_fmt(s['per_station_share'], 1)} %) — чеки на одну АЗС.",
                "check": "Чеки на одну АЗС по неделям и список закрытых и открытых объектов за оба месяца."},
               {"text": f"Чеки с НТУ изменились на {_fmt(s['ntu_pct'], 1)} %, объём топлива — на {_fmt(s['fuel_pct'], 1)} %: "
                        f"снижение может быть общим, а не только по одной составляющей.",
                "check": "Те же доли по группам сопоставимых АЗС: если движение везде одинаковое, причина общая."}]
        if s.get("ly_pct") is not None:
            why.append({"text": f"Год назад за те же дни изменение между этими месяцами составляло {_fmt(s['ly_pct'], 1)} % — "
                                f"возможно, часть движения сезонная.",
                        "check": "Та же пара месяцев за два-три прошлых года."})
        return {
    ...
    def potential_final(evidence: str) -> dict:
        ...
        return {
            ...
            "happened": [f"По пятнадцати объектам с наибольшим эффектом дополнительная выручка НТУ оценивается в {_fmt(s['top_revenue'])} руб. в месяц при текущем среднем чеке.",
                         "Отбор: высокий трафик (не меньше 3000 чеков за месяц) и конверсия ниже медианы сопоставимых АЗС."],
            "why": [],
            "where": ["Список объектов с разрывом и эффектом — в таблице; сравнение внутри группы регион × формат."],
            "actions": [{
                "action": "Можно рассмотреть разбор выкладки и предложения НТУ на кассе на объектах из верхних строк таблицы.",
                "basis": f"Конверсия НТУ ниже медианы своей группы у {s['below']} из {s['all']} крупных АЗС.",
                "effect": "Рост конверсии НТУ к медиане группы; оценка дополнительных чеков и выручки — в таблице.",
                "limits": "Оценка линейная; не подходит объектам с ограниченной площадью магазина или на ремонте.",
                "source": "анализ данных ответа", "confidence": "высокая"}],
...
def show(answer: pipeline.Answer, verbose: bool = False) -> None:
    ...
    if answer.analysis:
        ...
        marks = a.get("claims") or {}
        for title, key in (("Что произошло", "happened"), ("Почему", "why"), ("Где именно", "where"),
                           ("Что можно сделать", "actions"), ("Ограничения", "limitations")):
            if key == "actions" and a.get("recommendations") is not None:
                if a["recommendations"] or a.get("recommendationsWithheld"):
                    print(f"  {title}:")
                for rec in a["recommendations"]:
                    print(f"    • [Рекомендация, уверенность {rec['confidence']}] {rec['action']}")
                    print(f"      основание: {rec['basis']} | эффект: {rec['effect']} | не подходит: {rec['limits']} | источник: {rec['source']}")
                if not a["recommendations"] and a.get("recommendationsWithheld"):
                    print("    • Недостаточно данных для рекомендации.")
                if a.get("recommendationNote"):
                    print(f"    {a['recommendationNote']}")
                continue
            if a.get(key):
                print(f"  {title}:")
                for index, item in enumerate(a[key]):
                    claim = (marks.get(key) or [None] * (index + 1))[index] if key in marks else None
                    label = f"[{TYPE_TITLES.get(claim['type'], claim['type'])}] " if claim else ""
                    print(f"    • {label}{item}")
                    if claim and claim.get("formula"):
                        print(f"      формула: {'; '.join(claim['formula'])}")
    elif answer.summary:
