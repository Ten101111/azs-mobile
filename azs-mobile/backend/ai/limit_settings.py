...
def role_params() -> list[Param]:
    ...
    return [
        ...
        # Файлы и память папки (ИИ-07, ИИ-11).
        Param("folder_memory_chars", "Память папки", "знаков", min=200, max=5000,
              hint="Пожелания к ответам папки; область данных они не меняют"),
        Param("files_per_folder", "Файлов в диалоге или папке", "файлов", min=1, max=10,
              hint="Один файл — до 20 МБ (Р-5)"),
        Param("user_files_mb", "Объём файлов пользователя", "МБ", min=10, max=2000,
              hint="Сумма размеров загруженных файлов"),
    ]
...
def state(days: int = 7) -> dict:
    ...
    return {
        ...
        "planned": "Презентации, отложенные задачи и изображения появятся здесь вместе с ИИ-22, ИИ-24 и ИИ-19.",
    }
