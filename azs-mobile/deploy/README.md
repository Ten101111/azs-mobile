...
После первичной загрузки для обычного обновления достаточно:

```bash
npm run sync-kpi
```

Команда заново выгружает и заменяет только текущий месяц. Вся ранее загруженная история остается на сервере. Перед запуском нужно вручную подключить CheckPoint VPN через MobilePass+.

### Автоматическое обновление KPI на корпоративном Mac

После проверки ручного импорта настрой планировщик один раз:

```bash
./scripts/install_kpi_schedule.sh
```

Он обновляет текущий месяц каждые четыре часа: `00:15`, `04:15`, `08:15`, `12:15`, `16:15`, `20:15`.
Отдельная задача раз в пять минут проверяет доступность DWH. Если очередное обновление пропущено, она догружает KPI сразу после ручного подключения CheckPoint VPN. В VPN и MobilePass+ она не входит.

Логи: `~/Library/Logs/AZS Classifier/kpi-sync.log` и `~/Library/Logs/AZS Classifier/kpi-vpn-watch.log`.
